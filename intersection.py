...
class Intersection:
    ...
    def __init__(self, position, width, all_blocks):
    ...
    def pass_code(self):
        if self.n_in_rows <= self.n_out_rows:
            return [i for i in range(self.n_in_rows)]
        to_return = [random.randint(0, len(self.n_out_rows) - 1) for _ in range(self.n_in_rows)]
        return to_return

    def final_move(self, pass_code):
        for k in range(self.n_out_rows):
